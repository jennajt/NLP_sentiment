...
# Data
#data_1 = pd.read_csv('data/2_Muslim_data_1')
#data_2 = pd.read_csv('data/2_Muslim_data_2')
#data_3 = pd.read_csv('data/2_Muslim_data_3')
#data_4 = pd.read_csv('data/2_Muslim_data_4')

# 1: Muslim: Sentiment Across Newspapers Over Time, 2011-2022
def muslim_fig_1(data):
    fig = px.bar(data, y="Newspaper", x=['Positive','Neutral','Negative'], title="Sentiment Across Newspapers Over Time, 2011-2022", orientation='h', color_discrete_sequence=["green", "grey", "red"], animation_frame='year')
...
def muslim_fig_2(data):
    fig = go.Figure()

    fig.add_trace(go.Scatterpolar(
        r=[data['Joy'].iloc[0], data['Optimism'].iloc[0],data['Anger'].iloc[0], data_2['Sadness'].iloc[0]],
        theta=['Joy','Optimism','Anger','Sadness'],
        name='Daily Express'
    ))
    fig.add_trace(go.Scatterpolar(
        r=[data['Joy'].iloc[1], data['Optimism'].iloc[1],data['Anger'].iloc[1], data_2['Sadness'].iloc[1]],
        theta=['Joy','Optimism','Anger','Sadness'],
        name='Daily Mail'
    ))
    fig.add_trace(go.Scatterpolar(
        r=[data['Joy'].iloc[2], data['Optimism'].iloc[2],data['Anger'].iloc[2], data_2['Sadness'].iloc[2]],
        theta=['Joy','Optimism','Anger','Sadness'],
        name='The Sun'
    ))
